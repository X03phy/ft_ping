...
int icmp_send(s_icmp_pkt *pkt, int sockfd, struct sockaddr_in *addr)
{
	...
	ret = sendto(sockfd, pkt, sizeof(*pkt), 0, (struct sockaddr *)addr,
                     sizeof(*addr));
	if (ret == -1) {
		perror("sendto()");
		return (1);
...
int icmp_recv(size_t *out, int sockfd, char *buf, size_t len,
              struct sockaddr_in *from, struct timeval *tv_recv)
{
	socklen_t fromlen;
	ssize_t r;
